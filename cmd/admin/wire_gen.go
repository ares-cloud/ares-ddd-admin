...
func wireApp(bootstrap *configs.Bootstrap, configsData *configs.Data, storageConfig *configs.StorageConfig) (*app, func(), error) {
	...
	departmentCommandHandler := handlers2.NewDepartmentCommandHandler(iDepartmentRepository, departmentService)
	departmentQueryHandler := handlers2.NewDepartmentQueryHandler(iDepartmentRepository, departmentService)
	departmentController := rest2.NewDepartmentController(departmentCommandHandler, departmentQueryHandler, enforcer)
	baseServer := base.NewBaseServer(sysRoleController, sysUserController, sysTenantController, sysPermissionsController, authController, loginLogController, operationLogController, departmentController)
